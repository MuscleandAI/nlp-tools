...
datasets = ['20ng', 'R8', 'R52', 'ohsumed', 'mr', 'r8', 'my']
dataset = sys.argv[1]
class_list = [x.strip()
...
t_support = [ts.to(device) for ts in t_support]
...
def evaluate(features, labels, mask):
    ...
    with torch.no_grad():
        ...
        t_mask = torch.from_numpy(np.array(mask * 1., dtype=np.float32)).to(logits.device)
        ...
        pred = torch.max(logits, 1)[1].cpu()  # 将预测结果移动回CPU上
        labels = labels.to(logits.device)  # 将 labels 张量移动到与 logits 相同的设备上
        acc = ((pred.to(device) == torch.max(labels, 1)[1]).float() * t_mask).sum().item() / t_mask.sum().item()


    return loss.item(), acc, pred.numpy(), labels.cpu().numpy(), (time.time() - t_test)
...
for epoch in range(cfg.epochs):
    ...
    # Forward pass
    logits = model(t_features.to(device))
